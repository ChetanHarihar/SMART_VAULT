...
class EmployeePanel(tk.Frame):
    ...
    def init_ui(self):
        ...
        self.frame_container = tk.Frame(self, width=800, height=430)
        ...
        self.selection_frame = tk.Frame(self.frame_container, width=400, height=430, borderwidth=2, relief="groove", bg="white")
        ...
        self.category_treeview = TreeView(self.category_frame, show="tree", height=9)

        # Create columns
        self.category_treeview["columns"] = ("Category")
        self.category_treeview.column("#0", width=0, stretch=tk.NO)
        self.category_treeview.column("Category", width=340, anchor=tk.CENTER)
        ...
        self.item_label.pack(pady=(10, 5))

        self.item_treeview = TreeView(self.item_frame, height=8)
        ...
        self.category_btn = tk.Button(self.selection_btn_frame, text="Select Category", width=23, command=lambda: self.switch_frame(self.category_frame))
        self.category_btn.pack(side="left", padx=(0, 0.25))
        self.item_btn = tk.Button(self.selection_btn_frame, text="Select Item", width=23, command=lambda: self.switch_frame(self.item_frame))
        self.item_btn.pack(side="right", padx=(0.25, 0))

        # Frame to display data of the selected item 
        self.selected_item_frame = tk.LabelFrame(self.selection_frame, text="Item Details", width=400, height=100, borderwidth=2, relief="groove")
        self.selected_item_frame.pack_propagate(False)
        self.selected_item_frame.pack(pady=(0.5, 2.5), fill=tk.BOTH, expand=True)
        ...
        self.item_name_entry = tk.Entry(self.selected_item_frame, width=32)
        ...
        self.quantity_stock_entry = tk.Entry(self.selected_item_frame, width=32)
        self.quantity_stock_entry.grid(row=2, column=2, sticky='ew')

        # Widget to enter item quantity
        self.enter_quantity_spinbox = ttk.Spinbox(self.selected_item_frame, from_=0, to=0, width=25, font=('Arial', 10), foreground='#333333')
        ...
        self.add_btn = tk.Button(self.selected_item_frame, text="Add", width=10, relief="sunken", command=self.add_to_cart)
        self.add_btn.grid(row=3, column=2, sticky='se')

        # Create a frame to display the items in cart
        self.cart_frame = tk.Frame(self.frame_container, width=400, height=430, borderwidth=2, relief="groove", bg="white")
        ...
        self.cart_treeview = TreeView(self.cart_treeview_frame, height=13)
        ...
        self.remove_btn = tk.Button(self.cart_btn_frame, text="Remove", width=22, command=self.remove_item)
        self.remove_btn.pack(side="left", padx=(2.5, 1))
        self.pickup_btn = tk.Button(self.cart_btn_frame, text="Pickup", width=25, command=self.pickup)
    ...
    def load_pickup_frame(self):
        ...
        self.pickup_list_frame = tk.Frame(self.frame_container, width=400, height=400, borderwidth=2, relief='groove')
        ...
        self.items_label = tk.Label(self.grid_container, text="Items", font=("times new roman", 12), bg="#BEBEBE", width=34)
        self.items_label.grid(row=0, column=0)

        self.quantities_label = tk.Label(self.grid_container, text="Quantity", font=("times new roman", 12), bg="#BEBEBE", width=8)
        self.quantities_label.grid(row=0, column=1)

        self.check_label = tk.Label(self.grid_container, text="  ", font=("Arial", 12), bg="#BEBEBE", width=5)
        ...
        self.send_cmd_frame = tk.Frame(self.frame_container, width=400, height=400, borderwidth=2, relief='groove')
        self.send_cmd_frame.pack_propagate(False)
        self.send_cmd_frame.pack(side=tk.RIGHT, padx=(1.5, 0))

        self.pickup_data_label = tk.Label(self.send_cmd_frame, text="Press Start Session to\nstart collecting the items", font=("Arial", 11), highlightbackground="black", highlightthickness=2, width=30, height=5)
